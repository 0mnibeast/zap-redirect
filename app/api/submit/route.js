...
export async function GET(req) {
  ...
  const bg = decodeURIComponent(inUrl.searchParams.get('bg') || '#ff4f00');          // background color
  const msg = decodeURIComponent(inUrl.searchParams.get('msg') || 'Hang tight while we route your request…');
  const anim = (inUrl.searchParams.get('anim') || 'circle').toLowerCase();           // animation style
  const spinColor = decodeURIComponent(inUrl.searchParams.get('spinColor') || '#ffffff'); // spinner color
